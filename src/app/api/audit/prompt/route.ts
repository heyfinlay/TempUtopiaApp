...
import { auditPromptSchema } from "@/lib/validation/schemas";
...
export async function POST(request: NextRequest) {
  const limitResponse = enforceRateLimit(request, "audit-prompt", 20, 10 * 60_000);
  if (limitResponse) {
  ...
  const parsed = await parseJsonBody(request, auditPromptSchema);
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      prompt,
      summary: summarizeAuditText(prompt),
    })
